...
public class OpenLogPhaseListener implements PhaseListener {
	...
	public void beforePhase(PhaseEvent event) {
		...
		if (RENDER_RESPONSE == event.getPhaseId().getOrdinal()) {
			Map<String, Object> r = FacesContext.getCurrentInstance()
			...
			if (null != r.get("openLogBean")) {
				...
				OpenLogErrorHolder errList = (OpenLogErrorHolder) r
						.get("openLogBean");
				...
				if (null != r.get("error")) {
					...
					if (null != errList.getErrors()) {
						for (EventError error : errList.getErrors()) {
							...
							Level severity = convertSeverity(error
									.getSeverity());
							Document passedDoc = null;
							if (!"".equals(error.getUnid())) {
								try { 
									Database currDb = ExtLibUtil
											.getCurrentDatabase();
									passedDoc = currDb.getDocumentByUNID(error
											.getUnid());
								} catch (Exception e) {
									msg = msg
							...
							OpenLogItem.logErrorEx(error.getError(), msg,
									severity, passedDoc);
							try {
								passedDoc.recycle();
							} catch (Throwable e) {
					...
					if (null != errList.getEvents()) {
						for (EventError eventObj : errList.getEvents()) {
							...
							Level severity = convertSeverity(eventObj
									.getSeverity());
							Document passedDoc = null;
							...
							OpenLogItem
									.logEvent(null, msg, severity, passedDoc);
							try {
								passedDoc.recycle();
							} catch (Throwable e) {
								// nothing to recycle here, move along
	...
	private void processUncaughtException(Map<String, Object> r) {
		...
		String msg = "";
		if ("com.ibm.xsp.exception.EvaluationExceptionEx".equals(error
				.getClass().getName())) {
			...
			if (ee.getCause().getClass().getName().equals("InterpretException")) {
				InterpretException ie = (InterpretException) ee.getCause();
				msg = "Error on " + ee.getErrorComponentId() + " "
						+ ee.getErrorPropertyId() + " property/event, line "
						+ Integer.toString(ie.getErrorLine()) + ":\n\n"
						+ ie.getLocalizedMessage() + "\n\n"
						+ ie.getExpressionText();
			} else {
				if (ee.getCause().getClass().getName().equals("FacesException")) {

					FacesException fe2 = (FacesException) ee.getCause();

					msg = "Error on " + ee.getErrorComponentId() + " "
							+ ee.getErrorPropertyId() + " property/event "
							+ ":\n\n" + fe2.getLocalizedMessage() + "\n\n"
							+ fe2.getMessage();
				}
			}
		...
		} else if ("javax.faces.FacesException".equals(error.getClass()
				.getName())) {
			...
			try {
				...
				// ErrorOnMethod.xsp
				if (!"javax.faces.el.MethodNotFoundException".equals(fe
						.getCause().getClass().getName())) {
					if ("com.ibm.xsp.exception.EvaluationExceptionEx".equals(fe
							.getCause().getClass().getName())) {
						// Hit by ErrorOnClick.xsp
						ee = (EvaluationExceptionEx) fe.getCause();
					} else if ("javax.faces.el.PropertyNotFoundException"
							.equals(fe.getCause().getClass().getName())) {
						// Property not found exception, so error is on a
						// component property
						msg = "PropertyNotFoundException Error, cannot locate component:\n\n";
					} else if ("com.ibm.xsp.exception.EvaluationExceptionEx"
							.equals(fe.getCause().getCause().getClass()
									.getName())) {
					...
					if (null != ee) {
						msg = msg + ee.getErrorComponentId() + " "
								+ ee.getErrorPropertyId()
								+ " property/event:\n\n";
						if ("com.ibm.jscript.InterpretException".equals(ee
								.getCause().getClass().getName())) {
		...
		} else if ("com.ibm.xsp.FacesExceptionEx".equals(error.getClass()
				.getName())) {
			...
			try {
				if ("lotus.domino.NotesException".equals(fe.getCause()
						.getClass().getName())) {
					// sometimes the cause is a NotesException
					NotesException ne = (NotesException) fe.getCause();

					msg = msg + "NotesException - " + Integer.toString(ne.id)
							+ " " + ne.text;
				} else if ("java.io.IOException".equals(error.getClass()
						.getName())) {
					...
					EvaluationExceptionEx ee = (EvaluationExceptionEx) fe
							.getCause();
					InterpretException ie = (InterpretException) ee.getCause();
		...
		} else if ("javax.faces.el.PropertyNotFoundException".equals(error
				.getClass().getName())) {
