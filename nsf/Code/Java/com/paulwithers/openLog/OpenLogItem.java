...
public class OpenLogItem implements Serializable {
	...
	public static Database getLogDb() {
		if (_logDb == null) {
			try {
				_logDb = getSession().getDatabase(getThisServer(),
						getLogDbName(), false);
			} catch (Exception e) {
				debugPrint(e);
			}
	...
	public static Vector<String> getClientVersion() {
		if (_clientVersion == null) {
			...
			try {
				...
				if (cver != null) {
					if (cver.indexOf("|") > 0) {
						_clientVersion.addElement(cver.substring(0, cver
								.indexOf("|")));
						_clientVersion.addElement(cver.substring(cver
								.indexOf("|") + 1));
					} else {
	...
	public static String getLogDbName() {
		if ("".equals(_logDbName)) {
			...
			try {
				Document profile = getCurrentDatabase().getProfileDocument(
						"fmProfile", "");
				filePath = profile.getItemValueString("OpenLogDatabase");
				profile.recycle();
			...
			// _logDbName = getXspProperty("xsp.openlog.filepath",
			// "OpenLog.nsf");
			_logDbName = filePath;
	...
	public static Boolean getSuppressEventStack() {
		String dummyVar = getXspProperty("xsp.openlog.suppressEventStack",
				"false");
	...
	public static String getDisplayErrorGeneric() {
		if (null == _displayErrorGeneric) {
			_displayErrorGeneric = getXspProperty(
	...
	public static Document getErrDoc() {
		if (_errDoc != null) {
			try {
				...
				try {
					_errDoc = getCurrentDatabase().getDocumentByUNID(
							_errDocUnid);
				} catch (Exception e) {
					debugPrint(e);
				}
	...
	public static transient String olDebugLevel = getXspProperty(
	...
	private static String getXspProperty(String propertyName,
			String defaultValue) {
	...
	private static String getIniVar(String propertyName, String defaultValue) {
		try {
			String newVal = getSession().getEnvironmentString(propertyName,
					true);
	...
	public static String logError(Throwable ee) {
		...
		try {
			...
			FacesMessage m = new FacesMessage("Error in " + s[0].getClassName()
					+ ", line " + s[0].getLineNumber() + ": " + ee.toString());
	 ...
	 */
	public static String logErrorEx(Throwable ee, String msg,
			Level severityType, Document doc) {
		if (ee != null) {
			for (StackTraceElement elem : ee.getStackTrace()) {
				if (elem.getClassName().equals(OpenLogItem.class.getName())) {
					// NTF - we are by definition in a loop
	 ...
	 */
	public static String logEvent(Throwable ee, String msg, Level severityType,
			Document doc) {
		try {
			setMessage(msg);
	...
	/*
	 * A helper method that gets some basic information for the global variables
	 * that's common to all errors and events (event time and the name of the
	 * calling method).
	 * 
	 * The stacklevel parameter probably looks a little mysterious. It's
	 * supposed to be the number of levels below the calling method that we're
	 * at right now, so we know how far down the stack trace we need to look to
	 * get the name of the calling method. For example, if another method called
	 * the logError method, and the logError method called this method, then the
	 * calling method is going to be 2 levels down the stack, so stacklevel
	 * should be = 2. That may not make sense to anyone but me, but it seems to
	 * work...
	 */
	...
	public static void logError(Session s, Throwable ee, String message,
			Level severity, Document doc) {
		if (s != null) {
			_session = s;
		}
	...
	public static void logEvent(Session s, Throwable ee, String message,
			Level severity, Document doc) {
		if (s != null) {
			_session = s;
		}
	...
	public static boolean writeToLog() {
		...
		try {
			if (StringUtil.isEmpty(getLogEmail())) {
				...
				db = getSession().getDatabase(getThisServer(), "mail.box",
						false);
			}
			if (db == null) {
				System.out.println("Could not retrieve database at path "
						+ getLogDbName());
				return false;
			}
			...
			if (null != ee) {
				...
				if (ee instanceof NotesException) {
					logDoc.replaceItemValue("LogErrorNumber",
							((NotesException) ee).id);
					...
					errMsg = "Expression Language Interpret Exception "
				...
				if (TYPE_EVENT.equals(getEventType())) {
					if (!getSuppressEventStack()) {
						logDoc.replaceItemValue("LogStackTrace",
								getStackTrace(ee));
					}
				} else {
				...
				logDoc.replaceItemValue("LogFromMethod", ste.getClass() + "."
						+ ste.getMethodName());
			}
			...
			if (getErrDoc() != null) {
				...
				rtitem
						.appendText("The document associated with this event is:");
				rtitem.addNewLine(1);
	...
	public static void addFacesMessage(String component, String msg) {
		if (!"".equals(getDisplayErrorGeneric())) {
			if (null == ExtLibUtil.getRequestScope().get(
					"genericOpenLogMessage")) {
				ExtLibUtil.getRequestScope().put("genericOpenLogMessage",
						"Added");
			} else {
				return;
			}
