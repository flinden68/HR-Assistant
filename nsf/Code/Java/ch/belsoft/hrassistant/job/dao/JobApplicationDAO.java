...
ICrudDAO<JobApplication, String>, Serializable {
    ...
    public List<JobApplication> readWithKeys(String startKey, String endKey) {
        connectToService();
        return (List<JobApplication>) cloudantService.findAllDocumentFromViewKeys(
                JobApplication.class, DESIGN_DOC_KEYS, VIEW_NAME_KEYS, "STRING",
    ...
    public List<JobApplication> search(String query) {
        ...
        try {
            ...
            String queryFinal = SEARCH_PATTERN.replace(SEARCH_QUERYREPLACE,
                    query);
            result = (List<JobApplication>) cloudantService.search(SEARCH_INDEX,
                    JobApplication.class, SEARCH_COUNT, queryFinal);
