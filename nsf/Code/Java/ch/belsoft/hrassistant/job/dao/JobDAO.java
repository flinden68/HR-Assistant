...
Serializable {
    ...
    private static final String SEARCH_PATTERN_BY_COUNTRY = "companycountry:";
    private static final String SEARCH_PATTERN = "name:{QUERY} OR companyname:{QUERY} OR companystreet:{QUERY} OR companystreetnumber:{QUERY} OR companyzip:{QUERY} OR companycity:{QUERY} OR companycountry:{QUERY} OR jobtype:{QUERY} OR careerlevel:{QUERY} OR industry:{QUERY}";
    ...
    public List<Job> searchByCountry(String country){
        List<Job> result = new ArrayList<Job>();
        try {
            connectToService();
            String query = SEARCH_PATTERN_BY_COUNTRY+country;
            result = (List<Job>) cloudantService.search(SEARCH_INDEX,
                    Job.class, SEARCH_COUNT, query);
