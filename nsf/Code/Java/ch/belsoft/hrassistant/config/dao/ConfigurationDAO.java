...
public class ConfigurationDAO extends BaseDAO implements
	...
	public List<ConfigDefault> search(String query, String searchPattern) {
		List<ConfigDefault> result = new ArrayList<ConfigDefault>();
		try {
			connectToService();
			String queryFinal = searchPattern.replace(SEARCH_QUERYREPLACE,
					query);
	...
	public List<ConfigDefault> search(String query) {
		return this.search(query, SEARCH_PATTERN);
	}
}
