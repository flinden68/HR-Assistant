...
public class CloudantService {
    ...
    public List<?> findAllDocumentFromViewKeys(final Class<?> cls,
            final String designDoc, final String viewName,
            final String keyType, final int limit, final String startKey, final String endKey) {
        try {
            return connector.getDocumentConnector().findAllDocumentsFromViewKeys(
                    cls, designDoc, viewName, keyType, limit, startKey, endKey);
        } catch (Exception e) {
            CloudantLogger.CLOUDANT.getLogger().log(Level.SEVERE,
                    e.getMessage());
        }
    ...
    public List<?> search(final String searchIndexId, final Class<?> cls,
        ...
        try {
            connector.getQueryConnector().search(searchIndexId, cls,
                    queryLimit, query);
        } catch (Exception e) {
            CloudantLogger.CLOUDANT.getLogger().log(Level.SEVERE,
                    e.getMessage());
